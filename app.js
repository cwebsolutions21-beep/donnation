...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateLivePaymentUI(amount) {
        ...
        const qrUpiLink = buildQrUpiUri(amount, activeTxnId);
        ...
        if (phonepeLink) phonepeLink.href = buildAppUpiUri('phonepe', amount, activeTxnId);
        if (gpayLink) gpayLink.href = buildAppUpiUri('gpay', amount, activeTxnId);
        if (paytmLink) paytmLink.href = buildAppUpiUri('paytm', amount, activeTxnId);
        if (genericLink) genericLink.href = buildAppUpiUri('upi', amount, activeTxnId);
    }
    ...
    upiAppButtons.forEach(btn => {
        if (btn.element) {
            btn.element.addEventListener('click', (e) => {
                ...
                const deepLink = getPreciseMobileUpiDeepLink(btn.scheme, amount, activeTxnId);
    ...
    function getPreciseMobileUpiDeepLink(scheme, amount, trId) {
        const u = config.upiId.trim();
        const n = encodeURIComponent(config.payeeName.trim());
        const tn = encodeURIComponent(config.reasonNote.trim());
        const am = amount.toFixed(2);
        const mc = '8661'; // Merchant MCC
        
        const isAndroidDevice = /Android/i.test(navigator.userAgent);
        const isIOSDevice = /iPhone|iPad|iPod/i.test(navigator.userAgent);
        
        // Base standard UPI parameters
        const params = `pa=${u}&pn=${n}&am=${am}&tn=${tn}&tr=${trId}&mc=${mc}&mode=02&cu=INR`;
