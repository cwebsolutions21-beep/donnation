...
document.addEventListener('DOMContentLoaded', () => {
    ...
    adminForm.addEventListener('submit', (e) => {
        ...
        const smsMethodVal = adminSmsMethod ? adminSmsMethod.value : 'simulator';
        const smsApiKeyVal = adminInputSmsKey ? adminInputSmsKey.value.trim() : '';
        const smsDeviceIdVal = adminInputDeviceId ? adminInputDeviceId.value.trim() : '';
        const smsExtraAuthVal = adminInputExtraAuth ? adminInputExtraAuth.value.trim() : '';
        const smsSenderIdVal = adminInputSenderId ? adminInputSenderId.value.trim() : '';
    ...
    function sendSMS(phoneNumber, messageText) {
        ...
        if (method === 'macrodroid' && apiKey) {
            ...
            // Pass multiple query parameters (phone, number, message, msg) to guarantee
            // compatibility with whatever variable names they configured in MacroDroid.
            const encodedPhone = encodeURIComponent(formattedWithPlus);
            const encodedMsg = encodeURIComponent(messageText);
            
            // Build the webhook URL
            let webhookUrl = apiKey.trim();
            const joinChar = webhookUrl.includes('?') ? '&' : '?';
            webhookUrl = `${webhookUrl}${joinChar}phone=${encodedPhone}&number=${encodedPhone}&message=${encodedMsg}&msg=${encodedMsg}`;
