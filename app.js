...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Generate P2M UPI URI for general app links (essential for merchant accounts like BharatPe)
    function buildAppUpiUri(scheme, amount) {
        ...
        const params = `pa=${u}&pn=${n}&am=${am}&tn=${tn}&mc=8661&mode=02&cu=INR`;
    ...
    // Generate P2M unencoded UPI URI for QR codes (essential for merchant accounts like BharatPe)
    function buildQrUpiUri(amount) {
        const u = config.upiId.trim();
        const n = config.payeeName.trim();
        const tn = config.reasonNote.trim();
        const am = amount.toFixed(2);
        return `upi://pay?pa=${u}&pn=${n}&am=${am}&tn=${tn}&mc=8661&mode=02&cu=INR`;
    }
    ...
    // Helper to generate exact Direct-Launch UPI Deep Link matching standard P2M specifications
    function getPreciseMobileUpiDeepLink(scheme, amount) {
        ...
        // Base standard P2M UPI parameters
        const params = `pa=${u}&pn=${n}&am=${am}&tn=${tn}&mc=8661&mode=02&cu=INR`;
